...
On occasion, you may with to initialize a vector to be populated during later computation. This can be done with
...
This statement assigns the data in the raw datafile [my_input.dat]{style = "color: green;"} to the dataframe `df`. 
...
Download to your working directory, a csv file containing metadata for individuals to be used in later analyses. It is called [Tympo_ind_metadata.csv]{style ="color: green;"}.
...
```{r}
df[is.element(df$id, c('AA24300','AA24111','AA24303')),]
```
...
```{r}
summary(df[is.element(df$id, c('AA24300','AA24111','AA24303')),])
```
...
Your own functions can be used in the lapply() function to apply it to the elements of a list or vector. This gives then great utility.
...
```{r results = "hold"}
popNames(gl)
indNames(gl)[1:200]# Only first 200 entries shown
```
...
This will replace the existing population assignments with values of the individual metric year of capture. Confirm this using
